...
def download_steam_mod(mod_id, steamcmd_path, username, password, download_path):
    ...
    while code != 0 and counter < 3:
        ...
        if code != 0:
            click.echo(
                f"WARNING: Mod {title} download returned with code {code}!. Attempt {counter} of 3"
            )
    if counter == 3:
...
def save_modlines(manifest_url, mods_details, mods_path):
    ...
    for modline in mods_manifest:
        ...
        if set(mod_ids).difference(set(mods_details)):
            click.echo(
                f"WARNING: Not all mods for modline {modline} was downloaded, so it will be omitted!"
            )
            continue
        modlines[modline] = [
            mods_details[mod_id]["directory_name"] for mod_id in mod_ids
        ]
...
def update_mods(
    ...
    click.echo(
        f"Checking which of these mods to download: {[mod_details['title'] for mod_details in target_mod_details.values()]}..."
    )
    to_download = get_mods_to_download(target_mod_details, current_mods_details)
    if to_download:
        click.echo(
            f"Only need to downloading: {[target_mod_details[mod_id]['title'] for mod_id in to_download]}..."
        )
    else:
        return click.echo(
            f"No mods to download or update according to {Path(mods_path, MODS_DETAILS_PATH)}"
        )
    for mod_id in to_download:
        ...
        if not success:
            continue
        downloaded_dir = Path(
            download_path, "steamapps", "workshop", "content", "107410"
        )
        destination_dir = Path(mods_path)
        mod_dir_name = target_mod_details[mod_id]["directory_name"]
        click.echo(
            f"Making file and directory names safe: {target_mod_details[mod_id]['title']}..."
        )
